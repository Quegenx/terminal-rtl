// Locally modified for Terminal RTL: bounded, complete OSC dispatch. See PATCHES.md.
...
/// buffer. This local patch enforces the bound with and without `std`.
#[derive(Default)]
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn advance_osc_string<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        if self.osc_discard_escape && byte == b'\\' {
            if self.osc_overflow { performer.osc_rejected(); }
            else { self.osc_end(performer, 0x1b); }
            self.osc_raw.clear();
            self.osc_num_params = 0;
            self.osc_overflow = false;
            self.osc_discard_escape = false;
        ...
        if self.osc_discard_escape { self.osc_overflow = true; }
        if self.osc_overflow {
            self.osc_discard_escape = byte == 0x1b;
            if byte == 0x07 {
                performer.osc_rejected();
                self.osc_raw.clear();
                self.osc_num_params = 0;
                self.osc_overflow = false;
                self.state = State::Ground;
        ...
        match byte {
            0x00..=0x06 | 0x08..=0x17 | 0x19 | 0x1C..=0x1F => self.osc_overflow = true,
            0x07 => {
            ...
            0x1B => self.osc_discard_escape = true,
            0x3B => {
                // OSC 8 has exactly two separators; semicolons in the URI
                // are data, irrespective of the general OSC parameter limit.
                if self.osc_num_params >= 2 && self.osc_params[0] == (0, 1)
                    && self.osc_raw.first() == Some(&b'8') {
                    self.action_osc_put(byte);
                } else if self.osc_num_params >= MAX_OSC_PARAMS - 1 {
...
pub trait Perform {
    ...
    /// The complete OSC was rejected for malformed data or resource limits.
    fn osc_rejected(&mut self) {}
...
mod tests {
    ...
    fn parse_osc_max_params() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, format!("\x1b]{}\x1b\\", ";".repeat(MAX_OSC_PARAMS + 1)).as_bytes());
        assert!(dispatcher.dispatched.is_empty(), "never dispatch truncated parameters");
    }
    ...
    fn exceed_max_buffer_size() {
        ...
        for _ in 0..2048 {
            parser.advance(&mut dispatcher, &[b'a'; 4096]);
            assert!(parser.osc_raw.len() <= MAX_OSC_RAW);
            #[cfg(feature = "std")]
            assert!(parser.osc_raw.capacity() <= MAX_OSC_RAW.next_power_of_two());
        }
        parser.advance(&mut dispatcher, b"\x1b[2Jstill discarded\x1b\\OK");
        assert_eq!(dispatcher.dispatched, vec![Sequence::Print('O'), Sequence::Print('K')]);
    }
