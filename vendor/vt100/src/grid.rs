...
impl Grid {
    ...
    pub fn history_since(&self, total: u64) -> impl Iterator<Item = &crate::row::Row> {
        let count = self.scrollback_total.saturating_sub(total).min(self.scrollback.len() as u64) as usize;
        self.scrollback.iter().skip(self.scrollback.len() - count)
    }
