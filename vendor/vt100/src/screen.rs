...
impl Screen {
    ...
    /// Clear retained normal-screen history without erasing the visible screen.
    pub fn clear_scrollback(&mut self) {
        self.grid.clear_scrollback();
    }

    /// Number of normal-screen rows added to retained history since reset.
    #[must_use]
    pub fn scrollback_total(&self) -> u64 {
        self.grid.scrollback_total()
    }

    /// Retained normal-screen rows added after `total`, oldest first.
    /// Rows evicted by the configured history limit are not returned.
    pub fn history_since(&self, total: u64) -> impl Iterator<Item = Vec<crate::Cell>> + '_ {
        self.grid.history_since(total).map(|row| row.cells().cloned().collect())
    }
